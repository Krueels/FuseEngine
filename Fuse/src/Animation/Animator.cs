...
public sealed class Animator
{
    ...
    {
        Logger.Important($"[SkinnedDump] clip={(CurrentClip?.Name ?? "null")} t={TimeSeconds:F3} bones={_skeleton.Bones.Length} nodes={_skeleton.Nodes.Length}");
        ...
        Logger.Important($"[SkinnedDump] bones com |deltaT|>5: {exploded}/{worst.Count}");
        foreach (var w in worst.OrderByDescending(w => w.dT).Take(6))
        {
            Matrix4x4 f = FinalBoneMatrices[w.idx];
            Logger.Important($"[SkinnedDump] WORST '{w.name}' deltaT={w.dT:F2} deltaLin={w.dLin:F3} finalT=({f.M41:F2}, {f.M42:F2}, {f.M43:F2})");
        }
        ...
        if (worst.Count > 0 && _skeleton.Nodes.Length > 0)
        {
            ...
            Logger.Important($"[SkinnedChain] pior='{w0.name}' t={TimeSeconds:F3} ticks={TimeSeconds * tps:F1} dur={(CurrentClip != null ? CurrentClip.DurationTicks.ToString() : "?")}");
            int guard = 0;
            while (ni >= 0 && guard++ < 16)
            {
                var n = _skeleton.Nodes[ni];
                Logger.Important($"[SkinnedChain] '{n.Name}' localT=({n.Local.M14:F3},{n.Local.M24:F3},{n.Local.M34:F3}) restT=({n.RestLocal.M14:F3},{n.RestLocal.M24:F3},{n.RestLocal.M34:F3}) globalT=({n.Global.M14:F2},{n.Global.M24:F2},{n.Global.M34:F2})");
                ni = n.Parent;
            }
        }
    }
