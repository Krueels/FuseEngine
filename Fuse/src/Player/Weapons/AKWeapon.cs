...
public sealed class AKWeapon : IWeapon
{
    public string Id => "ak";
    ...
    public string ViewmodelIdleAnim => "AKS74U_Idle";
    public string ViewmodelFireAnim => "AKS74U_Fire1";

    public readonly string[] _fireAnims = Enumerable.Range(0, 2).Select(i => $"AKS74U_Fire{i}").ToArray();
    public string ViewmodelReloadAnim => "AKS74U_ReloadEmpty";
    public string ViewmodelReloadEmptyAnim => "AKS74U_ReloadEmpty2";
    public string ViewmodelDrawAnim => "AKS74U_Draw";

    public string ReloadAudioPath => Bible.Audio(Bible.AKReloadEmpty);
    ...
    public int ReserveAmmo { get; set; } = 280;
    public bool IsAutomatic => true;    // Semi-auto
    public float ReloadTime => 2.17f;
    ...
    private readonly string[] _fireSounds = Enumerable.Range(0, 4).Select(i => $"{Bible.AKFire}{i}.wav").ToArray();
    ...
    private void TransitionToIdle(bool crossfade = true)
    {
        ...
        {
            if (crossfade)
                _animator.CrossFade(_wasMoving ? "AKS74U_Walk" : "AKS74U_Idle", 0.5f);
            else
                _animator.Play(_wasMoving ? "AKS74U_Walk" : "AKS74U_Idle");
        ...
        switch (_animState)
        {
            ...
            case AnimState.Idle:
                ...
                if (isMoving != _wasMoving)
                {
                    if (_animator != null)
                    {
                        _animator.CrossFade(isMoving ? "AKS74U_Walk" : "AKS74U_Idle", 0.5f);
                        if (isMoving)
